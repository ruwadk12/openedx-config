...
echo "Dumping MySQL databases to ${BACKUPS_DIRECTORY}${SQL_FILE}" >> $OUTPUT_FILE
...
cp $(tutor config printroot)/data/mysql/dump.sql ${BACKUPS_DIRECTORY}${SQL_FILE}
...
tar -czf ${BACKUPS_DIRECTORY}openedx-mysql-${NOW}.tgz ${BACKUPS_DIRECTORY}${SQL_FILE}
...
# Remove the raw db dump since it is now tarballed
