...
# you only need to set these if you use password authentication for mongodb. 
...
PATH="$PATH:/home/linuxbrew/.linuxbrew/bin:/home/linuxbrew/.linuxbrew/sbin:/home/ubuntu/scripts:/home/ubuntu/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games:/snap/bin"
...
S3_BUCKET="courses.ruwadk12.com"
...
NOW=$(date +%Y%m%dT%H%M%S)
DEST=/home/ubuntu/backups/mongodb/mongo-dump-$NOW
CONTAINER=$(docker ps --format '{{.Names}}' | grep -m1 'mongodb')
sudo mkdir -p "$DEST"

tutor local exec mongodb rm -rf /data/db/dump.mongodb
tutor local exec mongodb mongodump --out=/data/db/dump.mongodb
sudo docker cp "$CONTAINER:/data/db/dump.mongodb/." "$DEST/"
...
# prune the uncompressed backup files
sudo rm -r $DEST
